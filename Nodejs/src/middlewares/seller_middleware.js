...
module.exports ={
initiateSeller: async function(req){
    let data={};
...
getAllSellerProducts: async function(req){
    const product = require('../models/product');
    let productData;
    if(req.params.pageNum==null) 
    {
        if(req.params.category==null)
        productData = await product.showSeller(connection,0,12,req.session.user["userID"]);
        else productData = await product.showCategorySeller(connection,0,12,req.params.category,req.session.user["userID"]);
    }
    else 
    {
        if(req.params.category==null)
        productData = await product.showSeller(connection,(req.params.pageNum-1)*12,12,req.session.user["userID"]);
        else productData = await product.showCategorySeller(connection,(req.params.pageNum-1)*12,12,req.params.category,req.session.user["userID"]);
    }
